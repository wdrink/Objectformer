...
from ObjectFormer.models.modules.conv_block import Deconv
...
class Localizer(nn.Module):
    def __init__(self, in_channel, output_channel):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channel, hidden_dim, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.2),
        )
    ...
    def forward(self, x, expected_h=None, expected_w=None):
        out = self.deconv1(x)
        out = self.conv1(out)
        ...
        if expected_h is not None:
            out = F.interpolate(out, (expected_h, expected_w), mode='bilinear', align_corners=True)
