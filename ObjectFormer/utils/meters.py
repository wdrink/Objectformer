...
from ObjectFormer.utils.distributed import is_dist_avail_and_initialized
import ObjectFormer.utils.logging as logging
from sklearn.metrics import roc_auc_score, top_k_accuracy_score, f1_score
...
class Metric():
    ...
    def synchronize_between_processes(self):
        ...
        self.all_labels = torch.cat(labels, dim=0).cpu()
        self.all_preds = torch.cat(preds, dim=0).cpu()
...
class AucMetric(Metric):
    def synchronize_between_processes(self):
        ...
        self.auc = roc_auc_score(self.all_labels, self.all_preds)


class TopKAccMetric(Metric):
    def __init__(self, k, num_gpus):
    ...
    def synchronize_between_processes(self):
        if not is_dist_avail_and_initialized():
            return
        super().synchronize_between_processes()
        self.acc = top_k_accuracy_score(self.all_labels, self.all_preds, k=self.k)
...
class F1Metric(Metric):
    def synchronize_between_processes(self):
        if not is_dist_avail_and_initialized():
            return
        super().synchronize_between_processes()
        self.f1 = f1_score(self.all_labels, self.all_preds)
